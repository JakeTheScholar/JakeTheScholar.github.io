...
const Accounts = {
  ...
  async render(el) {
    ...
    try {
      [this._accounts, this._journal] = await Promise.all([
        API.getAccounts(),
        API.getJournal(),
      ]);
    } catch (err) {
  ...
  _buildView(el) {
    ...
    html += '<div class="filter-chip' + (!Accounts.showArchived ? ' active' : '') + '" onclick="Accounts.showArchived=false;Accounts.selectedId=null;Accounts.editingId=null;Accounts._buildView(document.getElementById(\'content\'))">Active (' + activeAccounts.length + ')</div>';
    html += '<div class="filter-chip' + (Accounts.showArchived ? ' active' : '') + '" onclick="Accounts.showArchived=true;Accounts.selectedId=null;Accounts.editingId=null;Accounts._buildView(document.getElementById(\'content\'))">Archived (' + archivedAccounts.length + ')</div>';
    html += '</div>';
    ...
    if (Accounts.selectedId) {
      const acct = Accounts._getAccount(Accounts.selectedId);
      if (acct) {
        html += Accounts._detailPanel(acct, journal);
      }
    }
  ...
  _detailPanel(account, journal) {
    ...
    // Two columns: payout status + daily P&L
    ...
    // Payout History
    html += Accounts._payoutSection(account);
  ...
  _payoutSection(account) {
    let html = '';

    html += '<div style="background:var(--deep);border-radius:10px;padding:20px;margin-bottom:24px" id="payout-section-' + account.id + '">';
    ...
    html += '<span id="payout-total-' + account.id + '" style="font-family:\'DM Sans\',sans-serif;font-weight:800;font-size:18px;color:var(--green)">Loading...</span>';
    ...
    if (Accounts._showPayoutFormId === account.id) {
      ...
      html += '<button class="btn btn-ghost btn-sm" onclick="Accounts._showPayoutFormId=null;Accounts._buildView(document.getElementById(\'content\'))">Cancel</button>';
      html += '</div>';
    }

    html += '<div id="payout-list-' + account.id + '" style="color:var(--text-muted);font-size:13px">Loading payouts...</div>';
    html += '</div>';

    // Load payouts async
    Accounts._loadPayouts(account.id);

    return html;
  },

  async _loadPayouts(accountId) {
    try {
      const payouts = await API.getPayoutsForAccount(accountId);
      const totalPaid = payouts.reduce((s, p) => s + p.amount, 0);

      const totalEl = document.getElementById('payout-total-' + accountId);
      if (totalEl) totalEl.textContent = UI.currency(totalPaid);

      const listEl = document.getElementById('payout-list-' + accountId);
      if (!listEl) return;

      if (payouts.length === 0) {
        listEl.innerHTML = '<div style="color:var(--text-muted);font-size:13px">No payouts recorded yet</div>';
        return;
      }

      let html = '';
      const sorted = payouts.slice().sort((a, b) => b.date.localeCompare(a.date));
      sorted.forEach(p => {
      ...
      const listEl = document.getElementById('payout-list-' + accountId);
      if (listEl) listEl.innerHTML = '<div style="color:var(--red);font-size:13px">Failed to load payouts</div>';
    }
  },

  selectAccount(id) {
    Accounts.selectedId = Accounts.selectedId === id ? null : id;
    Accounts._buildView(document.getElementById('content'));
  },
  ...
  async createAccount() {
    ...
    try {
      await API.createAccount({ name, firm, phase, subtype: Accounts._hasSubtype(firm) ? subtype : null, status: 'active', startingBalance, trailingDrawdown: isNaN(trailingDrawdown) ? 2000 : trailingDrawdown, tradedBy, rules });
      Accounts.showAddForm = false;
      await Accounts.render(document.getElementById('content'));
    } catch (err) {
  ...
  async archiveAccount(id) {
    if (confirm('Archive this account? Trade history will be preserved.')) {
      try {
        const acct = Accounts._getAccount(id);
        if (acct) {
          await API.updateAccount(id, { ...acct, status: 'archived' });
        }
  ...
  async saveEdit(id) {
    ...
    try {
      await API.updateAccount(id, { name, firm, phase, subtype: Accounts._hasSubtype(firm) ? subtype : null, status: acct ? acct.status : 'active', startingBalance, trailingDrawdown: isNaN(trailingDrawdown) ? 2000 : trailingDrawdown, tradedBy, rules });
  ...
  async duplicateAccount(id) {
    const src = Accounts._getAccount(id);
    if (!src) return;
    ...
    const names = new Set(this._accounts.map(a => a.name));
    let n = this._accounts.filter(a => a.firm === src.firm && a.phase === src.phase).length + 1;
    ...
    try {
      const created = await API.createAccount({
        ...
        tradedBy: src.tradedBy || 'manual',
        rules: src.rules,
      });
      Accounts.selectedId = created.id;
      await Accounts.render(document.getElementById('content'));
    } catch (err) {
  ...
  async savePayout(accountId) {
    ...
    try {
      await API.createPayout({ accountId, date, amount, note });
      Accounts._showPayoutFormId = null;
      await Accounts.render(document.getElementById('content'));
    } catch (err) {
      alert('Failed to save payout: ' + err.message);
    }
  },
