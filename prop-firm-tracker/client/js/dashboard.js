...
const Dashboard = {
  ...
  async render(el) {
    ...
    let accounts, journal, payouts;
    try {
      [accounts, journal, payouts] = await Promise.all([
        API.getAccounts(),
        API.getJournal(),
        API.getPayouts(),
      ]);
    ...
    const totalEquity = active.reduce((sum, a) => {
    ...
    html += '<div class="stat-card" style="animation-delay:0ms"><div class="stat-label">Total Equity</div><div class="stat-value">' + UI.currency(totalEquity) + '</div><div style="font-size:12px;color:var(--text-secondary);margin-top:4px">across ' + active.length + ' account' + (active.length !== 1 ? 's' : '') + '</div></div>';
    ...
    // Payout Tracker
    const totalPayoutAmt = payouts.reduce((s, p) => s + p.amount, 0);
    if (totalPayoutAmt > 0 || payouts.length > 0) {
      const firmTotals = {};
      payouts.forEach(p => {
      ...
      html += '<div style="font-family:\'DM Sans\',sans-serif;font-weight:800;font-size:24px;color:var(--green)">' + UI.currency(totalPayoutAmt) + '</div>';
      html += '</div>';
    ...
    // Alerts
    const alerts = Rules.alerts(active, journal);
    alerts.forEach((a, i) => {
    ...
    if (alerts.some((a, i) => !Dashboard._dismissed.has(i))) {
      html += '<div style="margin-bottom:16px"></div>';
    }

    // Account cards by phase
    const funded = active.filter(a => a.phase === 'funded');
    const evals = active.filter(a => a.phase === 'eval');
    const live = active.filter(a => a.phase === 'live');
