...
// Client-side rules engine — identical to TCC, used for local computation

const Rules = {
  PRESETS: {
    apex_50k_eval: { name: 'Apex 50K Eval', startingBalance: 50000, profitTarget: 3000, trailingDrawdown: 2000, dailyLossLimit: null, consistencyPct: null, minTradingDays: 0, drawdownStopsAtTarget: true },
    apex_50k_funded: { name: 'Apex 50K Funded (PA)', startingBalance: 50000, profitTarget: null, trailingDrawdown: 2000, dailyLossLimit: null, consistencyPct: 0.50, minTradingDays: 8, drawdownStopsAtTarget: false },
    topstep_50k_normal: { name: 'Topstep 50K Normal XFA', startingBalance: 50000, profitTarget: null, trailingDrawdown: 2000, dailyLossLimit: 1000, consistencyPct: 0.50, minTradingDays: 5, minDayAmount: 150, drawdownStopsAtTarget: false },
    topstep_50k_consistency: { name: 'Topstep 50K Consistency XFA', startingBalance: 50000, profitTarget: null, trailingDrawdown: 2000, dailyLossLimit: 1000, consistencyPct: 0.40, minTradingDays: 3, minDayAmount: null, drawdownStopsAtTarget: false },
    mff_core_50k_eval: { name: 'MFF Core 50K Eval', startingBalance: 50000, profitTarget: 3000, trailingDrawdown: 2000, dailyLossLimit: null, consistencyPct: 0.50, minTradingDays: 5, drawdownStopsAtTarget: true },
    mff_core_50k_funded: { name: 'MFF Core 50K Funded', startingBalance: 50000, profitTarget: null, trailingDrawdown: 2000, dailyLossLimit: null, consistencyPct: 0.40, minTradingDays: 5, minDayAmount: null, drawdownStopsAtTarget: true },
    mff_rapid_50k_eval: { name: 'MFF Rapid 50K Eval', startingBalance: 50000, profitTarget: 3000, trailingDrawdown: 2000, dailyLossLimit: null, consistencyPct: 0.50, minTradingDays: 2, drawdownStopsAtTarget: true },
    mff_rapid_50k_funded: { name: 'MFF Rapid 50K Funded', startingBalance: 50000, profitTarget: null, trailingDrawdown: 2000, dailyLossLimit: null, consistencyPct: null, minTradingDays: 0, drawdownStopsAtTarget: true },
    alpha_50k_eval: { name: 'Alpha Futures 50K Eval', startingBalance: 50000, profitTarget: 3000, trailingDrawdown: 2000, dailyLossLimit: null, consistencyPct: 0.50, minTradingDays: 0, drawdownStopsAtTarget: true },
    alpha_50k_funded: { name: 'Alpha Futures 50K Funded', startingBalance: 50000, profitTarget: null, trailingDrawdown: 2000, dailyLossLimit: 1000, consistencyPct: 0.40, minTradingDays: 0, drawdownStopsAtTarget: true },
    lucid_50k_eval: { name: 'Lucid 50K Eval', startingBalance: 50000, profitTarget: 3000, trailingDrawdown: 2500, dailyLossLimit: null, consistencyPct: 0.50, minTradingDays: 2, drawdownStopsAtTarget: true },
    lucid_50k_funded: { name: 'Lucid 50K Funded', startingBalance: 50000, profitTarget: null, trailingDrawdown: 2500, dailyLossLimit: null, consistencyPct: null, minTradingDays: 5, minDayAmount: 150, drawdownStopsAtTarget: true },
  },
  ...
  dailyPnl(entries) {
    const byDate = {};
    entries.forEach(e => { byDate[e.date] = (byDate[e.date] || 0) + e.pnl; });
  ...
  compute(account, entries) {
    ...
    let runningBalance = startBal, highestBalance = startBal, trailingFloor = startBal - drawdownAmt;
    ...
    daily.forEach(d => {
      ...
      if (drawdownAmt > 0) {
        ...
        if (preset && preset.drawdownStopsAtTarget && preset.profitTarget) {
          ...
          if (highestBalance >= targetBalance) newFloor = Math.min(newFloor, targetBalance - drawdownAmt);
        }
    ...
    let dailyLossWarning = false, dailyLossHit = false;
    ...
    let consistencyOk = true, bestDayPct = 0, bestDayPnl = 0, bestDayDate = null, consistencyTarget = null;
    if (preset && preset.consistencyPct && totalPnl > 0) {
      const bestDay = daily.reduce((best, d) => d.pnl > best.pnl ? d : best, { pnl: 0, date: null });
    ...
    let evalProgress = null, projectedDays = null;
    ...
    return {
      currentBalance, totalPnl, highestBalance, trailingFloor, cushion,
      winRate, avgWin, avgLoss, profitFactor, tradingDays, todayPnl,
      dailyLossWarning, dailyLossHit,
      consistencyOk, bestDayPnl, bestDayPct, bestDayDate, consistencyTarget,
      evalProgress, projectedDays, payoutEligible,
      equityCurve, dailyPnl: daily,
    };
  },
  ...
  portfolioStats(accounts, journal, startDate, endDate) {
    ...
    let cumulative = 0;
    const equityCurve = dailyArr.map(d => { cumulative += d.pnl; return { date: d.date, balance: cumulative, pnl: d.pnl }; });
    return { totalPnl, winRate, avgDaily, tradingDays, bestDay, worstDay, equityCurve, tradeCount: filtered.length };
  },
  ...
  dateRange(range) {
    ...
    switch (range) {
      ...
      case 'week': { const d = new Date(now); d.setDate(d.getDate() - d.getDay() + 1); return { start: d.toISOString().slice(0, 10), end: today }; }
      case 'month': { const d = new Date(now.getFullYear(), now.getMonth(), 1); return { start: d.toISOString().slice(0, 10), end: today }; }
      case 'year': return { start: now.getFullYear() + '-01-01', end: today };
      case 'all': default: return { start: '2000-01-01', end: today };
    }
  },
};
